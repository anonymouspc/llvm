...
constexpr bool test() {
  ...
  // Test `views::chunk(view, n)`
  {
    ...
    std::same_as<std::ranges::chunk_view<std::span<int>>> decltype(auto) const_chunked = std::views::chunk(std::as_const(view), 2);
  ...
  // Test `view | views::chunk`
  {
    ...
    std::same_as<std::ranges::chunk_view<std::span<int>>> decltype(auto) const_chunked = std::as_const(view) | std::views::chunk(4);
